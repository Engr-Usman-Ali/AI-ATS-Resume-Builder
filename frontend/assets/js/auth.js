...
export async function handleForgotPassword(e) {
    ...
    if (!window.Utils.isValidEmail(email)) {
        window.Utils.showAlert('Please enter a valid email address', 'error');
        return;
    }
    
    if (!auth) { // Safety check
        window.Utils.showAlert('Configuration error. Refresh and try again.', 'error');
        return;
    }
    
    try {
        window.Utils.showLoading('reset-btn');

        // Modular Firebase Call: sendPasswordResetEmail(auth object, email)
        await sendPasswordResetEmail(auth, email); 
        
        window.Utils.hideLoading('reset-btn', originalText);
        window.Utils.showAlert('Password reset link sent! Check your email.', 'success');
        ...
        if (error.code === 'auth/user-not-found') {
            ...
            errorMessage = 'Invalid email address.';
             ...
             // For general Firebase errors
            errorMessage = `Reset failed: ${error.code.replace('auth/', '').replace(/-/g, ' ')}`;
        }
